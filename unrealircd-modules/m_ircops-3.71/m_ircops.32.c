...
DLLFUNC int MOD_INIT(m_ircops)(ModuleInfo *modinfo)
{
	...
	CommandAdd(modinfo->handle, MSG_HELPOPS, NULL, m_helpops, MAXPARA, M_USER);
...
int helpops = 0, aways = 0, total = 0;
	...
	sprintf(buf,
		...
		total, total > 1 ? "s" : "", aways);

	sendto_one(sptr, ":%s %d %s :%s", me.name, RPL_IRCOPS, sptr->name, buf);
	sendto_one(sptr, ":%s %d %s :End of /HELPOPS list", me.name, RPL_ENDOFIRCOPS, sptr->name);
